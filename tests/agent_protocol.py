...
class AgentProtocol(Agent):
    ...
    default_protocol = "echo YES | openssl s_client -connect localhost:8006 | grep TLSv1.2"
    ...
    def runTest(self):
        ...
        if self.error_code(self.tls1_1) is not 0:
            raise Exception(
                "%s return not '0' error code. This protocol should"
                "not be used" % self.tls1_1)  # TLS1_1 should be available for using

        if self.error_code(self.tls1_2) is not 0:
            raise Exception(
                "%s return not '0' error code. This protocol should"
                "not be used" % self.tls1_2)  # TLS1_2 should be available for using

        if self.error_code(self.tls1) is not 1:
            raise Exception(
                "%s return not '0' error code. This protocol should"
                "not be used" % self.tls1)  # TLS1 should NOT be available for using

        if self.error_code(self.sslv2) is not 1:  # sslv2 should NOT be available for using
            raise Exception("%s return not '1' error code. This protocol should"
                            "not be used" % self.sslv2)

        if self.error_code(self.sslv3) is not 1:  # sslv3 should NOT be available for using
            raise Exception("%s return not '1' error code. This protocol should"
                            "not be used" % self.sslv3)

        if self.error_code(self.dtls1) is not 1:  # dtls1 should NOT be available for using
            raise Exception("%s return not '1' error code. This protocol should"
